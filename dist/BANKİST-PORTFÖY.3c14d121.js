...
function obsCallbackSections(entries, observer) {
    const [entry] = entries;
    /* FİRST METHOD
  if (entry.isIntersecting) entry.target.classList.remove("section--hidden"); */ // SECOND METHOD
...
document.addEventListener('keydown', function(element) {
    if (element.key === 'Escape' && !modal.classList.contains('hidden')) closeModal();
});
...
navLinks.addEventListener("click", function(element) {
    ...
    if (element.target.tagName === "A") document.querySelector(id).scrollIntoView({
        behavior: "smooth"
    });
});
// SCROLL TO FİXED "nav"
...
//# sourceMappingURL=BANKİST-PORTFÖY.3c14d121.js.map
